...
import { KruuuFaucetPlugin } from 'app/plugins/kruuu_faucet/kruuu_faucet_plugin';
import { KruuuApiPlugin } from '~api/index';
...
export class StartCommand extends BaseStartCommand {
	static flags: StartFlags = {
		...
		'enable-kruuu-plugins': flagParser.boolean({
			description:
				'Enable KRUUU Plugins. Environment variable "KRUUU_ENABLE_PLUGINS" can also be used.',
			env: 'KRUUU_ENABLE_PLUGINS',
			default: false,
		}),
	...
	public async getApplication(config: PartialApplicationConfig): Promise<Application> {
		...
		if (flags['enable-kruuu-plugins']) {
			app.registerPlugin(new KruuuApiPlugin());
			app.registerPlugin(new KruuuFaucetPlugin());
		}
